...
public class IrisTest {
    ...
    public IrisTest() {
        List<String[]> irisDataset = Util.loadCSV("/СomputerScienceProblemsJava/Chapter7/data/iris.csv");
        Collections.shuffle(irisDataset);
        for (String[] iris : irisDataset) {
            double[] parameters = Arrays.stream(iris)
                    .limit(4).mapToDouble(Double::parseDouble)
                    .toArray();
    ...
    public Network<String>.Results classify() {
        Network<String> irisNetwork =
                new Network<>(new int[]{4, 6, 3}, 0.3, Util::sigmoid, Util::derivativeSigmoid);
