...
public class WineTest {
    ...
    public WineTest() {
        List<String[]> wineDataset = Util.loadCSV("/СomputerScienceProblemsJava/Chapter7/data/wine.csv");
    ...
    public Network<Integer>.Results classify() {
        Network<Integer> wineNetwork = new Network<>(
                new int[]{13, 7, 3}, 0.9, Util::sigmoid, Util::derivativeSigmoid);
        List<double[]> wineTrainers = wineParameters.subList(0, 150);
        List<double[]> wineTrainersCorrect = wineClassifications.subList(0, 150);
